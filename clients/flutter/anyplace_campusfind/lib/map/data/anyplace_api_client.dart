...
///
/// Ported from the map_refactor branch. The base URL comes from the app's own
/// `ApiConfig.serverUrl` (build-time `SERVER_URL` / default public UCY server),
/// preserving the current CampusFind API configuration.
class AnyplaceApiClient {
  ...
  AnyplaceApiClient({
    ...
    String? baseUrl,
        ...
        _baseUrl = baseUrl ?? ApiConfig.serverUrl;
  ...
  Future<List<SpaceModel>> fetchPublicSpaces() async {
    final uri = Uri.parse('$_baseUrl${ApiConfig.spacePublic}');
  ...
  Future<SpaceModel> fetchSpaceDetails(String buid) async {
    ...
    final uri = Uri.parse('$_baseUrl${ApiConfig.spaceGet}');
  ...
  Future<List<FloorModel>> fetchFloorsForBuilding(String buid) async {
    ...
    final uri = Uri.parse('$_baseUrl${ApiConfig.floorAll}');
  ...
  Future<String> fetchRadioMapMetadata(String buid, String floor) async {
    ...
    final uri = Uri.parse('$_baseUrl${ApiConfig.radiomapSpace}');
  ...
  Future<String> fetchRadioMapRaw(
    ...
    final normalizedPath =
        '${ApiConfig.radiomapFrozen}/$cleanBuid/$cleanFloor/$cleanFilename';
  ...
  Future<Uint8List> fetchFloorplanImage(String buid, String floor) async {
    ...
    final uri = Uri.parse(
      '$_baseUrl${ApiConfig.floorplans64}/$cleanBuid/$cleanFloor',
    );
