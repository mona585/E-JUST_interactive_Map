...
import 'package:anyplace_campusfind/ui/widgets/poi_marker.dart';
...
class FakePoiRepository implements PoiRepository {
  ...
  FakePoiRepository({this.pois = const {}});
...
void main() {
  ...
  Widget createTestWidget({
    ...
    final locService =
        locationService ?? FakeLocationService(fixedPosition: testGpsLocation);
        ...
        providers: [
          ChangeNotifierProvider<SpaceProvider>(
            create: (_) => SpaceProvider(
              ...
              poiRepository: poiRepository ?? FakePoiRepository(),
  ...
  group('MapScreen Widget Tests', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final firstMarker = tester.widget<BuildingMarker>(
          find.byType(BuildingMarker).first,
        );
        firstMarker.onTap();
        await tester.pumpAndSettle();
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final overlayLayer = tester.widget<OverlayImageLayer>(
          find.byType(OverlayImageLayer),
        );
        expect(overlayLayer.overlayImages.length, equals(1));
        final overlay = overlayLayer.overlayImages.first as OverlayImage;
        expect(overlay.bounds.southWest.latitude, equals(35.1440));
        expect(overlay.bounds.northEast.latitude, equals(35.1450));
      },
    );

    testWidgets(
      'selecting a floor displays POI markers and tapping POI opens PoiDetailCard',
      (tester) async {
        ...
        final repo = FakeSpaceRepository(testSpaces, fakeFloors: testFloors);
        final poiRepo = FakePoiRepository(
          pois: {
            'building_test_123/1': [samplePoi],
          },
        );

        await tester.pumpWidget(
          createTestWidget(repository: repo, poiRepository: poiRepo),
        );
        await tester.pumpAndSettle();

        // Request user location to show GPS marker
        final locationProvider = tester
            .element(find.byType(MapScreen))
            .read<LocationProvider>();
        await locationProvider.requestAndCenter();
        ...
        // Verify PoiMarker is rendered
        expect(find.byType(PoiMarker), findsOneWidget);
        expect(find.text('Conference Room 101'), findsOneWidget);

        // Tap on PoiMarker
        final poiMarkerWidget = tester.widget<PoiMarker>(
          find.byType(PoiMarker),
        );
        poiMarkerWidget.onTap();
        await tester.pump(const Duration(seconds: 1));

        // Verify PoiDetailCard is displayed with POI details
        expect(find.byType(PoiDetailCard), findsOneWidget);
        expect(find.text('Main presentation room'), findsOneWidget);

        // Verify user location marker remains top-most above POIs
        expect(find.byType(UserLocationMarker), findsOneWidget);
        await tester.pump(const Duration(seconds: 1));
      },
    );
    ...
    testWidgets(
      'requesting a route from a selected POI displays a PolylineLayer on the map',
      (tester) async {
        const samplePoi = PoiModel(
          puid: 'poi_test_1',
          buid: 'building_test_123',
        ...
        final navigationRepo = FakeNavigationRepository()
          ..routeToReturn = const NavigationRouteModel(
            points: [
              NavigationRoutePoint(
                latitude: 35.1443,
                longitude: 33.4103,
                puid: 'start_connector',
                buid: 'building_test_123',
                floorNumber: '1',
                poisType: 'None',
              ),
        ...
        final mockNativeService = FakeNativePositioningService();
        final locProvider = LocationProvider(
          locationService: FakeLocationService(fixedPosition: testGpsLocation),
          nativePositioningService: mockNativeService,
        );
        final spaceProvider = SpaceProvider(
          repository: FakeSpaceRepository(testSpaces, fakeFloors: testFloors),
          radioMapRepository: FakeRadioMapRepository(),
          floorplanRepository: FakeFloorplanRepository(),
        ...
        await tester.pumpWidget(
          MaterialApp(
            ...
            home: MultiProvider(
              providers: [
                ChangeNotifierProvider<SpaceProvider>.value(
                  value: spaceProvider,
                ),
        ...
        spaceProvider.selectSpace(testSpaces.first);
        await tester.pumpAndSettle();
        spaceProvider.selectFloor(testFloors['building_test_123']!.last);
        await tester.pumpAndSettle();

        await locProvider.requestAndCenter();
        mockNativeService.controller.add(
          PositionEstimate(
            latitude: 35.1443,
            longitude: 33.4103,
            buid: 'building_test_123',
        ...
        spaceProvider.selectPoi(samplePoi);
        await tester.pump();
        await tester.ensureVisible(find.text('Route Here'));
        await tester.tap(find.text('Route Here'));
        await tester.pump(const Duration(milliseconds: 300));

        expect(find.byType(PolylineLayer), findsOneWidget);
        expect(find.text('Route ready on floor 1'), findsOneWidget);
    ...
    testWidgets(
      'displays Indoor Wi-Fi badge and styled UserLocationMarker when native indoor estimate is active',
      (tester) async {
        final mockNativeService = FakeNativePositioningService();
        final repo = FakeSpaceRepository(testSpaces, fakeFloors: testFloors);
        ...
        // Acquire GPS position initially
        await locProvider.requestAndCenter();
        await tester.pump(const Duration(seconds: 1));

        // Select Building 123 -> Floor 1
        spaceProvider.selectSpace(testSpaces.first);
        await tester.pump(const Duration(milliseconds: 500));
        spaceProvider.selectFloor(testFloors['building_test_123']!.last);
        await tester.pump(const Duration(seconds: 1));
        ...
        final validIndoorEst = PositionEstimate(
          latitude: 35.1444,
          longitude: 33.4105,
          buid: 'building_test_123',
          floor: '1',
        ...
        mockNativeService.controller.add(validIndoorEst);
        await tester.pump(const Duration(milliseconds: 200));

        // Verify Header Badge displays Indoor Wi-Fi (6 APs)
        expect(find.textContaining('Indoor Wi-Fi (6 APs)'), findsOneWidget);

        // Verify UserLocationMarker is rendered with indoor style
        final marker = tester.widget<UserLocationMarker>(
          find.byType(UserLocationMarker),
        );
        expect(marker.isIndoor, isTrue);
        locProvider.dispose();
      },
    );
  });
...
class _ThrowingSpaceRepository implements SpaceRepository {
  ...
  _ThrowingSpaceRepository({required this.onFetch, required this.errorMessage});
